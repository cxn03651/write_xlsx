...
module Writexlsx
  module Utility
    ...
    #
    def position_object_pixels(worksheet, col_start, row_start, x1, y1, width, height, anchor = nil) # :nodoc:
      # Adjust start column for negative offsets.
      while x1 < 0 && col_start > 0
        x1 += size_col(worksheet, col_start - 1)
        col_start -= 1
      end

      # Adjust start row for negative offsets.
      while y1 < 0 && row_start > 0
        y1 += size_row(worksheet, row_start - 1)
        row_start -= 1
      end
      ...
      x_abs = if @col_size_changed
                (0..col_start - 1).inject(0) { |sum, col| sum += size_col(worksheet, col, anchor) }
              else
      ...
      y_abs = if @row_size_changed
                (0..row_start - 1).inject(0) { |sum, row| sum += size_row(worksheet, row, anchor) }
              else
      ...
      while x1 >= size_col(worksheet, col_start, anchor)
        x1 -= size_col(worksheet, col_start)
        col_start += 1
      end

      # Adjust start row for offsets that are greater than the row height.
      while y1 >= size_row(worksheet, row_start, anchor)
        y1 -= size_row(worksheet, row_start)
        row_start += 1
      end
      ...
      width  += x1 if size_col(worksheet, col_start, anchor) > 0
      height += y1 if size_row(worksheet, row_start, anchor) > 0

      # Subtract the underlying cell widths to find the end cell of the object.
      while width >= size_col(worksheet, col_end, anchor)
        width -= size_col(worksheet, col_end, anchor)
        col_end += 1
      end

      # Subtract the underlying cell heights to find the end cell of the object.
      while height >= size_row(worksheet, row_end, anchor)
        height -= size_row(worksheet, row_end, anchor)
        row_end += 1
      end
    ...
    #
    def size_col(worksheet, col, anchor = 0) # :nodoc:
      # Look up the cell value to see if it has been changed.
      if worksheet.col_info[col]
        width  = worksheet.col_info[col].width || @default_col_width
        hidden = worksheet.col_info[col].hidden
    ...
    #
    def size_row(worksheet, row, anchor = 0) # :nodoc:
      # Look up the cell value to see if it has been changed
      if worksheet.row_sizes[row]
        height, hidden = worksheet.row_sizes[row]
        ...
        pixels = (4 / 3.0 * worksheet.default_row_height).to_i
      end
      pixels
    end
  end
